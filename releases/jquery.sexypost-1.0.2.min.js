...
function send(form,action,method,async){var data=new FormData();var fields=$(form).serializeArray();$.each(fields,function(){data.append($(this).attr("name"),$(this).val());});$("input:file",form).each(function(){var files=this.files;for(i=0;i<files.length;i++)data.append($(this).attr("name"),files[i]);});xhr.open(method,action,async);xhr.setRequestHeader("Cache-Control","no-cache");xhr.setRequestHeader("X-Requested-With","XMLHttpRequest");xhr.send(data);}});return this;}})(jQuery);