 ...
 */
(function($){$.fn.sexyPost=function(options){var events=["onstart","onprogress","oncomplete","onerror","onabort","onfilestart","onfilecomplete"];var config={async:true,onstart:function(event){},onprogress:function(event,completed,loaded,total){},oncomplete:function(event,responseText){},onerror:function(event){},onabort:function(event){}};if(options)$.extend(config,options);this.each(function(){for(event in events){if(config[events[event]]){$(this).bind("sexyPost."+events[event],config[events[event]]);}}
...
xhr.onload=function(event){form.trigger("sexyPost.oncomplete",[xhr.responseText]);}
